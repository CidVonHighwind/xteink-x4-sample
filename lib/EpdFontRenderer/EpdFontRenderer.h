...
template <typename Renderable> class EpdFontRenderer {
  Renderable *renderer;
  void renderChar(uint32_t cp, int *x, const int *y, uint8_t color);

public:
  const EpdFont *font;
  explicit EpdFontRenderer(const EpdFont *font, Renderable *renderer);
  ~EpdFontRenderer() = default;
  void renderString(const char *string, int *x, int *y, uint8_t color);
};

inline int uncompress(uint8_t *dest, size_t uncompressedSize,
                      const uint8_t *source, size_t sourceSize) {
  if (uncompressedSize == 0 || dest == nullptr || sourceSize == 0 ||
      source == nullptr) {
    return -1;
  }
  tinfl_init(&decomp);
  ...
  const tinfl_status decomp_status = tinfl_decompress(
      &decomp, source, &sourceSize, dest, dest, &uncompressedSize,
...
EpdFontRenderer<Renderable>::EpdFontRenderer(const EpdFont *font,
                                             Renderable *renderer) {
  this->font = font;
...
template <typename Renderable>
void EpdFontRenderer<Renderable>::renderString(const char *string, int *x,
                                               int *y, const uint8_t color) {
...
template <typename Renderable>
void EpdFontRenderer<Renderable>::renderChar(const uint32_t cp, int *x,
                                             const int *y, uint8_t color) {
  ...
  if (font->data->compressed) {
    ...
    uncompress(tmpBitmap, bitmapSize, &font->data->bitmap[offset],
               glyph->compressedSize);
    bitmap = tmpBitmap;
  } else {
